...
class JMOAB_ATCart(object):
	...
	def __init__(self, NS):
		...
		rospy.init_node('jmoab_ros_atcart_omni_node', anonymous=True)
	...
	def send_four_wheels(self, sbus_front_left, sbus_front_right, sbus_rear_left, sbus_rear_right):

		FL_bytes = self.sbus2word(sbus_front_left)
		FR_bytes = self.sbus2word(sbus_front_right)
		RL_bytes = self.sbus2word(sbus_rear_left)
		RR_bytes = self.sbus2word(sbus_rear_right)

		## combine as 4 elements [str_H, str_L, thr_H, thr_L]
		all_bytes = FL_bytes+FR_bytes+RL_bytes+RR_bytes
	...
	def cmd_callback(self, msg):
		if len(msg.data) == 4:
			cmd_front_left = msg.data[0]
			cmd_front_right = msg.data[1]

			cmd_rear_left = msg.data[2]
			cmd_rear_right = msg.data[3]
			...
			self.send_four_wheels(cmd_front_left, cmd_front_right, cmd_rear_left, cmd_rear_right)
			self.callback_timestamp = time.time()
	...
	def loop(self):
		...
		while not rospy.is_shutdown():
			...
			if ((time.time() - self.callback_timestamp) > self.callback_timeout):
				self.send_four_wheels(int(self.sbus_mid), int(self.sbus_mid), int(self.sbus_mid), int(self.sbus_mid))
