...
from std_msgs.msg import Int16MultiArray, UInt8, Int8MultiArray, Float32MultiArray, Int16
...
class JMOAB_ATCart(object):
	...
	def __init__(self, NS, params_file):

		rospy.init_node('jmoab_ros_pwmcart_node', anonymous=True)
		...
		sv_node = 'pwmcart_params'
		self.sbus_left_max_db = rosparam.get_param(sv_node+"/sbus_left_max_db")
		self.sbus_right_max_db = rosparam.get_param(sv_node+"/sbus_right_max_db")
		self.sbus_left_min_db = rosparam.get_param(sv_node+"/sbus_left_min_db")
		self.sbus_right_min_db = rosparam.get_param(sv_node+"/sbus_right_min_db")
		self.show_log = rosparam.get_param(sv_node+"/show_log")

		rospy.loginfo("Using parameters below")
		rospy.loginfo("sbus_left_max_db: {}".format(self.sbus_left_max_db))
		rospy.loginfo("sbus_right_max_db: {}".format(self.sbus_right_max_db))
		rospy.loginfo("sbus_left_min_db: {}".format(self.sbus_left_min_db))
		rospy.loginfo("sbus_right_min_db: {}".format(self.sbus_right_min_db))
		...
		servo_topic = self.namespace_attaching(NS, '/jmoab/servo')
		...
		rospy.Subscriber(servo_topic, Int16, self.servo_callback)
		...
		rospy.loginfo("Subscribing on {:} topic as std_msgs/Int16  ex: 1120-1520-1920 ranges ".format(servo_topic))
...
if __name__ == '__main__':
	...
	parser.add_argument('--params_file',
						help="A file path of PwmCartParams.yaml, default is the one in jmoab-ros/cfg/")
	...
	if params_file is None:
		print("Use jmoab-ros/cfg/PwmCartParams.yaml")
		rospack = rospkg.RosPack()
		jmoab_ros_path = rospack.get_path("jmoab-ros")
		yaml_name = "PwmCartParams.yaml"
		yaml_path = os.path.join(jmoab_ros_path, "cfg", yaml_name)
	else:
		yaml_path = params_file
		print("Use {:}".format(params_file))


	jmoab = JMOAB_ATCart(ns, yaml_path)